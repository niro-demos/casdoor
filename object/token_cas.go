...
// GenerateCasPgt generates a new proxy-granting-ticket id. The id is not
// stored/activated yet - it only becomes valid and redeemable once the
// caller separately invokes StoreCasTokenForPgt for it. This split lets
// CasP3ProxyValidate send the id to the pgtUrl callback first, and only
// activate it after that callback confirms the caller actually controls the
// callback endpoint (see the pgtUrl handling in controllers/cas.go).
func GenerateCasPgt() string {
	return fmt.Sprintf("PGT-%s", util.GenerateId())
}

// StoreCasTokenForPgt activates pgt (previously generated by GenerateCasPgt),
// making it valid and redeemable via GetCasTokenByPgt / the CAS "/proxy"
// endpoint. Callers must only do this after confirming the pgtUrl callback
// succeeded - storing/activating it earlier would let any service-ticket
// holder obtain a redeemable proxy-granting ticket without ever proving
// control of a reachable callback endpoint.
func StoreCasTokenForPgt(pgt string, token *CasAuthenticationSuccess, service, userId string) {
