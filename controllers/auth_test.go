...
// test for TC-37F50BB2: HandleLoggedIn's ResponseTypeCas branch must refuse
// to mint a CAS service ticket for a "service" that is not one of the
// target application's registered RedirectUris -- the same allow-list
// object.CheckCasLogin already enforces in the GET /api/get-app-login
// pre-check, but which is never applied at the point the ticket is actually
// minted (POST /api/login?type=cas).
//
...
func TestHandleLoggedIn_CasTicketRequiresRegisteredService(t *testing.T) {
	...
	attackerService := "https://totally-unregistered-attacker-domain.example/collect"
	c := newHandleLoggedInController("service=" + url.QueryEscape(attackerService) + "&type=cas")
...
// database-dependent full ticket-issuance flow exercised above, proving the
// failure above is about the unregistered service specifically, not a
// broken environment.
