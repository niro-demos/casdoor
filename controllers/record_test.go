...
var recordTestInitOnce sync.Once

// initRecordTestApp brings up the same DB connection and API route table the
// real server (main.go) wires up, in-process, so this test can drive
// controllers.ApiController the way a real client would: over HTTP, through
// beego's own router and session middleware.
func initRecordTestApp() {
	recordTestInitOnce.Do(func() {
		object.InitConfig()
...
type recordTestAPIResponse struct {
	Status string          `json:"status"`
	Msg    string          `json:"msg"`
	Data   json.RawMessage `json:"data"`
}

type recordTestRecordDTO struct {
	Id           int    `json:"id"`
	Organization string `json:"organization"`
	Action       string `json:"action"`
}

// doRecordTestRequest performs one in-process HTTP round trip against
// beego's own router, carrying cookies forward like a real browser session.
func doRecordTestRequest(t *testing.T, method, path string, body []byte, cookies []*http.Cookie) (*http.Response, []byte) {
	t.Helper()
...
// setupOrgAdminWithRecords creates a fresh organization, application, and
// org-scoped (non-global) admin user, plus audit-log Records seeded directly
// in the admin's own organization and in a sibling organization the admin
// must never be able to read - mirroring the ORG_ADMIN / niro-test /
// built-in scenario from TC-75A318DC's proof-of-concept.
func setupOrgAdminWithRecords(t *testing.T) (orgName, appName, adminUsername, adminPassword, otherOrgName string) {
	t.Helper()

	suffix := fmt.Sprintf("%d", time.Now().UnixNano())
	orgName = "tc75a318dc-org-" + suffix
	otherOrgName = "tc75a318dc-other-" + suffix
	appName = "tc75a318dc-app-" + suffix
	adminUsername = "tc75a318dc-admin-" + suffix
	...
	// Seed audit-log records directly (bypassing the async HTTP record
	// pipeline main.go wires up outside routers.InitAPI()) in both the
	// admin's own organization and a sibling organization the admin must
	// never see, matching the cross-tenant leak the PoC demonstrated.
	...
	seedRecord(orgName, "tc75a318dc-own-record-a-"+suffix)
	seedRecord(orgName, "tc75a318dc-own-record-b-"+suffix)
	seedRecord(otherOrgName, "tc75a318dc-other-record-"+suffix)

	return orgName, appName, adminUsername, adminPassword, otherOrgName
}

func recordTestLogin(t *testing.T, username, password, organization, application string) []*http.Cookie {
	t.Helper()
	...
	resp, raw := doRecordTestRequest(t, http.MethodPost, "/api/login", body, nil)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("login HTTP status = %d, body = %s", resp.StatusCode, raw)
	}

	var lr recordTestAPIResponse
...
// test for TC-75A318DC: an organization-scoped (non-global) admin must only
// ever see audit-log records belonging to their own organization, whether or
// not pagination params (pageSize/p) are supplied to GET /api/get-records.
func TestGetRecordsUnpaginatedIsScopedToCallerOrganization(t *testing.T) {
	...
	cookies := recordTestLogin(t, adminUsername, adminPassword, orgName, appName)
	...
	pagResp, pagRaw := doRecordTestRequest(t, http.MethodGet, "/api/get-records?pageSize=50&p=1", nil, cookies)
	if pagResp.StatusCode != http.StatusOK {
		t.Fatalf("paginated GET /api/get-records HTTP status = %d, body = %s", pagResp.StatusCode, pagRaw)
	}
	var pagEnvelope recordTestAPIResponse
	...
	var pagRecords []recordTestRecordDTO
	...
	unpagResp, unpagRaw := doRecordTestRequest(t, http.MethodGet, "/api/get-records", nil, cookies)
	...
	var unpagEnvelope recordTestAPIResponse
	...
	var unpagRecords []recordTestRecordDTO
	...
	if len(unpagRecords) == 0 {
		t.Fatalf("unpaginated request returned zero records; cannot validate the invariant (expected at least the admin's own seeded records)")
	}
